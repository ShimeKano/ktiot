-- Sailo Peace - Auto Teleport to Saved Position with Tween + Auto Key Spam (Fixed v2)
-- Fix: Dùng keyboard input thực tế thay vì SendKeyEvent
...
-- Dùng _G để savedCInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
...
-- Lấy HumanoidRootPart của nhân vật hiện tại (luFrame và trạng thái auto tồn tại qua các lần respawn / re-execute
if _G.SailoPeace_IsAutoEnabled == nil then
    _G.SailoPeace_IsAutoEnabled = false
end
if _G.SailoPeace_LoopId == nil then
...
local function calculateôn cập nhật)
local function getRoot()
    local char = player.Character
    if char then
        return char:FindFirstChild("HumanoidRootPart")
...
-- Hàm ấn phím - Dùng InputBegan event (cách mới, không dùng SendKeyEvent)
...
-- Cách khác: Trigger InputBegan/InputEnded trực tiếp
local function pressKeyDirect(key)
    local keyCode = Enum.KeyCode[key]
    if keyCode thenTweenTime(fromCFrame, toCFrame)
    local distance = (fromCFrame.Position - toCFrame.Position).Magnitude
    return distance / TWEEN_SPEED
end

-- Hàm ấn phím - Cách mới: Tạo InputObject giả
...
-- Hàm ấn phím - Cách 2: Tự gọi game event
local function pressKeyAlt(keyName)
    local keyCode = Enum.KeyCode[keyName]
    if keyCode then
        -- Fire event trực tiếp
...
saveBtn.Size = UDim2.new(02.new(1, 0, 0, 45)
title.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
title.Text = "🌊 Sailo Peace - Auto Teleport + KeySpam"
title.TextColor3 = Color3.new(1, 1, 1)
title.TextScaled = true
...
dropdown.TextColor3 = Color3.new(1,1,1).9, 0, 0, 45)
saveBtn.Position = UDim2.new(0.05, 0, 0, 55)
saveBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
saveBtn.Text = "💾 Lưu Vị Trí Hiện Tại"
saveBtn.TextColor3 = Color3.new(1,1,1)
...
toggleBtn.Size = UDim2.new(0.9, 0, 0,.Text = "⏱️ Thời gian delay: " .. item.text
        optionsFrame.Visible = false
    end)
end
...
status.Size = UDim2.new(0.9, 0, 0, true
lockBtn.Parent = mainFrame
...
local function startLockPosition()
    ...
    lockConnection = RunService.Heartbeat:Connect(function()
        if not _G.SailoPeace_LockPosition or not _G.SailoPeace_Save 45)
status.Position = UDim2.new(0.05, 0, 0, 330)
status.BackgroundTransparency = 1
status.Text = "Trạng thái: Chưa lưu vị trí\n"
status.TextColor3 = Color3.fromRGB(200, 200, 200)
...
lockBtn.MouseButton1Click:Connect(function()
    ...
    if _G.SailoPeace_LockPosition then
        if not _G.SailoPeace_SavedCFrame then
            ...
            _G.SailoPeace_LockPosition = false
            task.wait(2)
            status.Text = "Trạng thái: ChoPeace_SavedCFrame then
            status.Text = "❌ Chưa lưu vị trí!\n"
            _G.SailoPeace_LockPosition = false
            task.wait(2)
            status.Text = "Trạng thái: Chưa lưu vị trí"
...
keySpamBtn.MouseButton1Click:Connect(function()
    ...
    if _G.SailoPeace_KeySpamEnabled then
        ...
        status.Text = "⌨️ Auto Key Spam chạy\n(CXZVF spam liên tục)"
        startKeySpamLoop()
        print("✅ Key Spam bắt đầu - Ấn CXZVF liên tục")
    else
        keySpamBtn.Text = "🟠 Spam bắt đầu - Ấn CXZVF liên tục (Event-based)")
    else
        keySpamBtn.Text = "🟠 BẬT AUTO KEY SPAM"
        keySpamBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 50)
...
if _G.SailoPeace_IsAutoEnabled and _G.SailoPeace_SavedCFrame then
    toggleBtn.Text = "🟢 ĐANG AUTO BAY VỀ"
    toggleBtn.BackgroundColor3 = Color BẬT AUTO KEY SPAM"
        keySpamBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 50)
        status.Text = "❌ Auto Key Spam tắt"
        print("❌ Key Spam dừng")
    end
...
print("✅ Sailo Peace v2 - Fixed SendKeyEvent Error!")
print("📝 Dùng InputBegan/InputEnded event thay vì SendKeyEvent")
